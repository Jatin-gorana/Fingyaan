...
const NewsPage = () => {
  ...
  // GNews API Key
  const apiKey = 'f09857c2793ae8a6ddc0ff14e0d63759'; // Your provided API key
  ...
  const fetchNews = async (category) => {
    const query = category === 'All' ? 'finance' : category.toLowerCase();
    const url = `${baseUrl}/search?q=${query}&token=${apiKey}`;

    try {
      const response = await axios.get(url);
      const filteredNews = response.data.articles.filter((article) => article.image); // Filter out articles without images
